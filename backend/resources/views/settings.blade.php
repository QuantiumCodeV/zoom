...
@endif</span
        ...
        <!-- End Page Header -->
          ...
          <div class="card card-lg">
            ...
            <div class="card-body">
              <div class="row">
              <div class="col-sm-6">
                  <label for="expectedValueNewProjectLabel" class="form-label">Chat ID Телеграмма</label>

                  <!-- Form -->
                  <div class="mb-4">
                    <div class="input-group input-group-merge">
                      
                      <input type="text" class="form-control" name="chatid" id="expectedValueNewProjectLabel" placeholder="Enter value here" aria-label="Enter value here" value="{{ $settings->chatId }}">
                    </div>
                  </div>
                  <!-- End Form -->
                </div>
                ...
                <div class="col-sm-6">
                  <label for="expectedValueNewProjectLabel" class="form-label">Токен бота</label>

                  <!-- Form -->
                  <div class="mb-4">
                    <div class="input-group input-group-merge">
                      
                      <input type="text" class="form-control" name="tokenBot" id="expectedValueNewProjectLabel" placeholder="Enter value here" aria-label="Enter value here" value="{{ $settings->telegramBotToken }}">
                    </div>
                  </div>
                  <!-- End Form -->
                </div>
                <!-- End Col -->
                <div class="col-sm-6">
                  <label for="expectedValueNewProjectLabel" class="form-label">Файл для MacOS</label>

                  <!-- Form -->
                  <div class="input-group mb-3">
                    <input type="text" class="form-control" name="macos_file" value="{{ $settings->macos_file }}">
  <input type="file" class="form-control" id="mac_os_input">
</div>
                  <!-- End Form -->
                </div>
                <!-- End Col -->
                ...
                <div class="col-sm-6">
                  <label for="expectedValueNewProjectLabel" class="form-label">Файл для Windows</label>

                  <!-- Form -->
                  <div class="input-group mb-3">
                    <input type="text" class="form-control" name="windows_file" value="{{ $settings->windows_file }}">
                    <input type="file" class="form-control" id="windows_file_input">
                  </div>
                  <!-- End Form -->
                </div>
                <div class="col-sm-6">
  ...
  <input type="file" class="form-control" id="mac_os_modal_input">
                ...
                <div class=" d-flex justify-content-end gap-3">
                  ...
                  <button type="button" class="btn btn-primary" id="settingsFirstBlock">Сохранить изменения</button>
                </div>
                <!-- End Col -->
              </div>
              </div>
    ...
    <script>
                  ...
                  $('#mac_os_input').on('change', function() {
    var formData = new FormData();
    formData.append('file', this.files[0]);
    formData.append('_token', '{{ csrf_token() }}');

    $.ajax({
        url: "{{ route("settings.upload") }}", // заменил на ваш роут
        method: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
          console.log(response)
            // Обновите поле ввода с именем 'macos_file' с путем, возвращенным сервером.
            $('input[name="macos_file"]').val(response.file);
        },
...
$('#mac_os_modal_input').on('change', function() {
    var formData = new FormData();
    formData.append('file', this.files[0]);
    formData.append('_token', '{{ csrf_token() }}');
    ...
    $.ajax({
        ...
        success: function(response) {
          console.log(response)
            // Обновите поле ввода с именем 'macos_file' с путем, возвращенным сервером.
            $('input[name="macos_file_modal"]').val(response.file);
        },
        error: function(response) {
            console.error(response);
        },
...
$('#windows_file_input').on('change', function() {
    var formData = new FormData();
    formData.append('file', this.files[0]);
    formData.append('_token', '{{ csrf_token() }}');
    ...
    $.ajax({
        ...
        success: function(response) {
          console.log(response)
            // Обновите поле ввода с именем 'macos_file' с путем, возвращенным сервером.
            $('input[name="windows_file"]').val(response.file);
        },
        error: function(response) {
            console.error(response);
        },
...
$('#windows_file_modal_input').on('change', function() {
    var formData = new FormData();
    formData.append('file', this.files[0]);
    formData.append('_token', '{{ csrf_token() }}');
    ...
    $.ajax({
        ...
        success: function(response) {
          console.log(response)
            // Обновите поле ввода с именем 'macos_file' с путем, возвращенным сервером.
            $('input[name="windows_file_modal"]').val(response.file);
        },
        error: function(response) {
            console.error(response);
        },
